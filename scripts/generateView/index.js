// 生成模板文件的脚本，生成路径src/view，生成文件js,css,vue
...
process.stdin.on('data', async chunk => {
    ...
    // css文件
    const cssFile = resolve(componentPath, 'index.css')
    // 判断组件文件夹是否存在
    ...
    try {
        ...
        log(`正在生成 entry 文件 ${entryFile}`)
        await generateFile(cssFile, cssTemplate(componentName))
        successLog('生成成功')
    } catch (e) {
        errorLog(e.message+componentName)
    }
